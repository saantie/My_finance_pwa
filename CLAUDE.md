...
```typescript
{
  ...
  shared_with: string[]          // [] = private, ["email@..."] = selective share (v2.0 Cloud sync)
}
```
...
#### F4.1+ — v2.0 features
- Cloud sync (Firebase Firestore, opt-in)
- Investment / กองทุนรวม tracking
- Net worth dashboard
- Goal-based saving
- **Family sharing — Selective Account**
  - share ระดับ account: แต่ละบัญชีมี `shared_with[]` — เลือกได้ว่า share กับใครบ้าง
  - คนรับเห็นเฉพาะ transaction ของบัญชีที่ share ไม่เห็น account อื่น
  - UX: Settings → บัญชี → toggle "แชร์บัญชีนี้" → ใส่ email
  - ต้องรอ Cloud sync v2.0 (Firebase) ก่อน — local-first ทำ sharing ตรงๆ ไม่ได้
  - ไม่มี share ระดับ category ใน v2.0 (เกิน scope)
...
| PDF parsing | pdf.js | Industry standard |
| Voice | Web Speech API + custom Thai NLP | Free, on-device |
| Charts | Chart.js | Mature, fast |
| Storage v1.0 | localStorage | Local-first |
| Storage v2.0 | Firebase Firestore (opt-in) | Free tier, no OAuth verification needed |
...
| 2026-05 | Selective account sharing ระดับ account (ไม่ใช่ category) | Simpler scope, ต้องรอ Cloud sync v2.0 |
